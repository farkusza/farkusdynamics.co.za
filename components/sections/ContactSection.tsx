...
export default function ContactSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent(
      `New enquiry from ${formState.name}`
    );
    ...
    window.location.href = `mailto:hello@farkusdynamics.co.za?subject=${subject}&body=${body}`;
    setSubmitted(true);
  };
  ...
  return (
    ...
    >
      {/* Background accent */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-fd-violet/3 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          >
            <span className="text-sm font-medium text-fd-violet tracking-wider uppercase mb-4 block">
              Get in Touch
            </span>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 group cursor-pointer">
                <div className="w-10 h-10 rounded-lg bg-fd-violet/10 flex items-center justify-center group-hover:bg-fd-violet/20 transition-colors">
                  <Mail size={18} className="text-fd-violet" />
                </div>
                <div>
                  <div className="text-sm text-fd-text-3">Email</div>
                  <a
                    href="mailto:hello@farkusdynamics.co.za"
                    className="text-fd-text hover:text-fd-violet transition-colors"
                  >
            ...
            {submitted ? (
              <div className="h-full flex items-center justify-center p-12 rounded-2xl border border-fd-border bg-fd-surface/50">
                <div className="text-center">
                  ...
                  <p className="text-fd-text-2">
                    Your email client should open now. Send it to reach us!
                  </p>
                </div>
              </div>
            ) : (
              <form
                ...
                className="p-8 sm:p-10 rounded-2xl border border-fd-border bg-fd-surface/50 space-y-6"
              >
                <div>
                  <label
                    htmlFor="name"
                    className="block text-sm font-medium text-fd-text-2 mb-2"
                  >
                    Name
                  </label>
                  <input
                ...
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-fd-violet hover:bg-fd-violet-light text-white font-semibold transition-all duration-300 hover:shadow-lg hover:shadow-fd-violet/25 hover:-translate-y-0.5"
                >
                  Send Message
                  <ArrowRight size={18} />
                </button>
